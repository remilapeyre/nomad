...
OUTER:
	for {
		...
		currentPreemptions := iter.ctx.Plan().NodePreemptions[option.Node.ID]
		maxParallel := 0
		if iter.taskGroup.Migrate != nil {
			maxParallel = iter.taskGroup.Migrate.MaxParallel
		}
		...
		for _, task := range iter.taskGroup.Tasks {
			...
			if len(taskResources.Networks) > 0 {
				...
				if offer == nil {
					...
					preemptedAllocsForTask, networkResourcesMet := preemptForNetworkResourceAsk(iter.priority, proposed, taskResources, netIdx, currentPreemptions, maxParallel)

					if !networkResourcesMet {
						iter.ctx.Metrics().ExhaustedNode(option.Node,
							fmt.Sprintf("unable to meet network resource %v after preemption", ask))
						netIdx.Release()
						continue OUTER
					}
					allocsToPreempt = append(allocsToPreempt, preemptedAllocsForTask...)

					// First subtract out preempted allocations
					proposed = structs.RemoveAllocs(proposed, preemptedAllocsForTask)
		...
		if !fit {
			...
			preemptForTaskGroup := findPreemptibleAllocationsForTaskGroup(iter.priority, current, total, option.Node, currentPreemptions, maxParallel)
