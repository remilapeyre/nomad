...
func getPreemptionScore(resource *structs.Resources, resourceAsk *structs.Resources, preemptionType PreemptionType, maxParallel int, numPreemptedAllocs int) float64 {
	...
	if maxParallel > 0 && numPreemptedAllocs > maxParallel {
		maxParallelScorePenalty = float64(numPreemptedAllocs-maxParallel) * maxParallelPenalty
	}
...
func findPreemptibleAllocationsForTaskGroup(jobPriority int, current []*structs.Allocation, resourceAsk *structs.Resources, node *structs.Node, currentPreemptions []*structs.Allocation, maxParallel int) []*structs.Allocation {
	...
	for _, allocGrp := range allocsByPriority {
		for len(allocGrp.allocs) > 0 && !allRequirementsMet {
			...
			if closestAllocIndex == -1 {
				...
				// Can break to go to the next group
				break
			}
...
// computeCurrentPreemptions counts the number of other allocations being preempted that match the alloc under consideration
// This is used as a scoring factor to minimize too many allocs of the same job being preempted at once
...
func preemptForNetworkResourceAsk(jobPriority int, currentAllocs []*structs.Allocation, resourceAsk *structs.Resources,
	netIdx *structs.NetworkIndex, currentPreemptions []*structs.Allocation, maxParallel int) ([]*structs.Allocation, bool) {
	...
	for device, currentAllocs := range deviceToAllocs {
		...
		for _, allocsGrp := range allocsByPriority {
			...
			sort.Slice(allocs, func(i, j int) bool {
				currentPreemptionCount1 := computeCurrentPreemptions(allocs[i], currentPreemptions)
				distance1 := getPreemptionScore(allocs[i].Resources, resourceAsk, NetworkResource, maxParallel, currentPreemptionCount1)

				currentPreemptionCount2 := computeCurrentPreemptions(allocs[j], currentPreemptions)
				distance2 := getPreemptionScore(allocs[j].Resources, resourceAsk, NetworkResource, maxParallel, currentPreemptionCount2)
